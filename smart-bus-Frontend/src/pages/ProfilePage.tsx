...
const ProfilePage = () => {
  ...
  if (loading) return <div className="p-8 text-center text-app-mu">{t('loading_profile')}</div>;
  if (!profile) return <div className="p-8 text-center text-app-err">{t('failed_load_profile')}</div>;

  return (
    <div className="mx-auto max-w-2xl p-6 pb-20 font-sans animate-in fade-in zoom-in-95 duration-300">
      <header className="mb-8">
        <h1 className="text-2xl font-black text-app-tx">{t('my_profile')}</h1>
        <p className="text-xs text-app-mu">{t('profile_subtitle')}</p>
      </header>
      ...
      <div className="rounded-2xl border border-app-bd bg-app-card p-6 shadow-sm mb-6">
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="text-center md:text-left space-y-1">
            ...
            <p className="text-xs font-bold text-app-mu uppercase tracking-widest">{profile.student_id || t('student')}</p>
      ...
      <div className="rounded-2xl border border-app-bd bg-app-card p-6 shadow-sm space-y-4">
        <h4 className="mb-4 flex items-center gap-2 text-[13px] font-bold uppercase tracking-wider text-app-tx">
          <Ic.Pin className="text-app-am" size={16} /> {t('account_details')}
        </h4>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-app-card2 border border-app-bd/50">
            <p className="text-[10px] font-black uppercase tracking-widest text-app-mu mb-1">{t('email')}</p>
            <p className="text-sm font-bold text-app-tx break-all">{profile.email}</p>
          </div>
          <div className="p-4 rounded-xl bg-app-card2 border border-app-bd/50">
            <p className="text-[10px] font-black uppercase tracking-widest text-app-mu mb-1">{t('phone')}</p>
            <p className="text-sm font-bold text-app-tx">{profile.phone || t('not_provided')}</p>
          </div>
          <div className="p-4 rounded-xl bg-app-card2 border border-app-bd/50 md:col-span-2">
            <p className="text-[10px] font-black uppercase tracking-widest text-app-mu mb-1">{t('joined')}</p>

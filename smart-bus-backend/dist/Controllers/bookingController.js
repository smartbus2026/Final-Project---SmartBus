...
exports.getAssignedTrips = exports.recoverCancelledBookings = exports.dispatchBus = exports.getDemandAggregation = exports.getTodayBookings = exports.getBookingStats = exports.closeTrip = exports.markAttendanceStatus = exports.updateBooking = exports.cancelBooking = exports.getAllBookings = exports.getMyBookings = exports.createBooking = void 0;
...
const getMyBookings = async (req, res) => {
    try {
        ...
        const rawBookings = await Booking_model_1.default.find({ user: user.id, date: { $gte: todayStart } })
            .populate("route")
            .lean()
            .sort("-createdAt");
...
const dispatchBus = async (req, res) => {
    try {
        ...
        const actualRouteIds = routeId ? [routeId] : (routeIds || []);
        ...
        const actualAssignments = assignments && assignments.length > 0
            ? assignments
        ...
        for (const assignment of actualAssignments) {
            ...
            const bus = await Bus_1.default.findById(currentBusId);
            if (!bus) {
            ...
            if (!finalDriverId) {
                ...
                const fallbackDriver = await User_1.default.findOne({ role: "driver" });
                if (!fallbackDriver) {
            ...
            const existingTripForDriver = await Trip_1.default.findOne({
            ...
            const existingTripForBus = await Trip_1.default.findOne({
        ...
        const previousDistinctBuses = await Booking_model_1.default.distinct("busId", {
            date: { $gte: targetDate, $lte: dayEnd },
            timeSlot: timeSlot,
        ...
        const allPendingBookings = await Booking_model_1.default.find(bookingQuery);
        const assignmentsWithBookings = validatedAssignments.map(val => ({
            ...val,
            bookings: []
        }));
        for (const booking of allPendingBookings) {
            const availableAssignment = assignmentsWithBookings.find(a => a.bookings.length < a.capacity);
        ...
        const results = await Promise.all(assignmentsWithBookings
            ...
            const routeTripUpserts = await Promise.all(actualRouteIds.map(async (routeId) => {
                const passengerCount = bookings.filter(b => b.route?.toString() === routeId).length;
                return await Trip_1.default.findOneAndUpdate({
                    route: routeId,
                    driver: driverId,
                    date: targetDate,
                    time_slot: tripTimeSlot,
            ...
            await Promise.all(userIds.map(userId => notification_1.default.create({
                user: userId,
                title: "Bus Assigned",
...
const getAssignedTrips = async (req, res) => {
    try {
        ...
        const assignedBookings = await Booking_model_1.default.find(query)
            .populate("route", "name")
            .populate({
            path: "busId",
        ...
        const Trip = (await Promise.resolve().then(() => __importStar(require("../models/Trip")))).default;
        for (const group of tripsArray) {
            ...
            if (realTrip) {
                ...
                group.driverName = realTrip.driver?.name || "Unassigned";
            }
        }
        res.status(200).json({
            status: "success",
...
exports.getAssignedTrips = getAssignedTrips;
